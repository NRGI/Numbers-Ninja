getEitiData <- function(pathToFolder) {
    ...
    for(i in 1:length(listOfFiles)) {
        ...
        if(extractData) {
            ...
            maxRows <- max(which(str_detect(str_to_lower(tempCompanyData$`Government revenues from extractive companies, per revenue stream`), "notes"))) - 1
